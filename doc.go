...
/*
Package acmrpcclient implements a websocket-enabled Bitcoin JSON-RPC client.

Overview

This client provides a robust and easy to use client for interfacing with a
Bitcoin RPC server that uses a acmd/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with acmd (https://github.com/Actinium-project/acmd),
acmwallet (https://github.com/Actinium-project/acmwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, acmd and
acmwallet provide a websocket interface that is more efficient than the standard
...
acmd or acmwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by acmd and
acmwallet only uses a single connection that remains open and allows
...
All notifications provided by acmd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by acmd (and
acmwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that acmd intentionally separates the wallet
functionality into a separate process named acmwallet.  This means if you are
connected to the acmd RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, acmwallet provides pass through treatment
...
detected by type asserting the error in a *acmjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*acmjson.RPCError); ok {
  		switch jerr.Code {
  		case acmjson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - acmdwebsockets
   ...
   block count
 - acmwalletwebsockets
   Connects to a acmwallet RPC server using TLS-secured websockets, registers
